...
CPPSRCS += win/nsOSHelperAppService.cpp

# We build files in 'unified' mode by including several files
# together into a single source file.  This cuts down on
# compilation times and debug information size.
UNIFIED_CPPSRCS := Unified_cpp_uriloader_exthandler0.cpp
CPPSRCS += $(UNIFIED_CPPSRCS)
misc::
	$(call py_action,buildlist,$(DEPTH)/dist/bin/chrome.manifest 'manifest components/nsHandlerService.manifest')
misc::
	$(call py_action,buildlist,$(DEPTH)/dist/bin/chrome.manifest 'manifest components/nsWebHandlerApp.manifest')
RTL_FLAGS += -MD
LIBRARY_NAME := uriloader_exthandler
FORCE_STATIC_LIB := 1
REAL_LIBRARY := uriloader_exthandler.lib
OS_LIBS += psapi.lib
OS_LIBS += shell32.lib
OS_LIBS += dbghelp.lib
