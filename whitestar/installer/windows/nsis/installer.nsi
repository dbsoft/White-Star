...
Section "-Application" APP_IDX
  ...
  ; On install always add the WhiteStarHTML and WhiteStarURL keys.
  ; An empty string is used for the 5th param because WhiteStarHTML is not a
  ; protocol handler.
  ${GetLongPath} "$INSTDIR\${FileMainEXE}" $8
  StrCpy $2 "$\"$8$\" -osint -url $\"%1$\""

  ; In Win8, the delegate execute handler picks up the value in WhiteStarURL and
  ; WhiteStarHTML to launch the desktop browser when it needs to.
  ${AddDisabledDDEHandlerValues} "WhiteStarHTML" "$2" "$8,1" \
                                 "${AppRegName} Document" ""
  ${AddDisabledDDEHandlerValues} "WhiteStarURL" "$2" "$8,1" "${AppRegName} URL" \
                                 "true"
